...
# 基础模型类（保持不变）
class VLLMTaskSoundBackgroundModel(torch.nn.Module):
    """
    ...
    def get_prompt(self, task_type="matching", video_path=None, instruction=None):
        ...
        if task_type == "QA":
            ...
            background_knowledge = instruction.get("background_knowledge")
            ...
            user_prompt = (
                f"Here's a humorous video. And you'll be given a background konwledge of the video. "
                f"Based on the its visual, audio information and background konwledge, answer the following question: {question}\n\n"
                "Output format:\n"
                "Answer: <answer>\n\n"
                f"Background Knowledge: {background_knowledge}"
            )
            ...
            user_prompt =  (f"Along with visual and audio information in the video and background knowledge of the video. And {question}\n"
