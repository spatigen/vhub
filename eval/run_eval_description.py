...
import csv
import sys
from vllm_task_description import VLLMTaskDescriptionModel
...
from backbone import Qwen25_Omni_Description
from backbone import Qwen25_VL_Description
from backbone import Gemini25_Description
...
def load_backbone(TESTING_MODEL):
    ...
    if TESTING_MODEL == "Qwen2.5-Omni":
        model = Qwen25_Omni_Description()
    elif TESTING_MODEL == "Qwen2.5-VL":
        model = Qwen25_VL_Description()
    elif TESTING_MODEL == "Gemini2.5-flash":
        model = Gemini25_Description()
    elif TESTING_MODEL == "GPT-4o":
        model = GPT4o_Description()
    elif TESTING_MODEL == "InterVL 3.5":
        model = InternVL35_Description()
    elif TESTING_MODEL == "Minicpm 2.6-o":
        model = MINICPM26_o_Description()
    elif TESTING_MODEL == "GLM 4.5V":
        model = GLM45V_Description()
    elif TESTING_MODEL == "video SALMONN 2":
        model = VideoSALMONN2_Description()
    else:
...
def main():
    ...
    model = VLLMTaskDescriptionModel(backbone, device="cuda")
