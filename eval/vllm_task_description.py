...
# 基础模型类（保持不变）
class VLLMTaskDescriptionModel(torch.nn.Module):
    """
    ...
    def generate(self, instruction, video_path):
        ...
        # --- 调用 API 获取响应 ---
        raw_response = self.model.get_completion(system_prompt, user_prompt)
    ...
    def get_prompt(self, task_type="matching", video_path=None, instruction=None):
        ...
        if task_type == "QA":
            ...
            user_prompt = (
                f"You'll be given a description of the video."
            ...
            user_prompt = (
                f"You will also be given a description of the video. "
            ...
            user_prompt = (f"You'll be given a description of the video. And {question}\n"
...
if __name__ == "__main__":
    ...
    model = VLLMTaskDescriptionModel(model, device="cuda")
    metrics = {}
