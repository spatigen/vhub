...
# 基础模型类（保持不变）
class VLLMTaskModel(torch.nn.Module):
    """
    ...
    def get_prompt(self, task_type="matching", video_path=None, instruction=None):
        ...
        if task_type == "QA":
            ...
            user_prompt = (
                ...
                f"Based on these frames, answer the following question: {question}\n\n"
                "Output format:\n"
                "Answer: <answer>\n\n"
            )
            ...
            user_prompt =  (f"Along with the frames from the video. And {question}\n"
                "Please respond with response with the option letter only.\n\n"
                "Output format:\n"
                "Answer: <answer>\n\n")
            
        elif task_type == "Process_QA":
            
            system_prompt = "You are a helpful AI assistant specialized in video understanding and humor analysis. You can generate high-quality question-answer pairs based on video content, video descriptions and humorous explanations."
            user_prompt = instruction['user_prompt']

        elif task_type == "Process_matching":
            
            system_prompt = f"""You are a helpful AI assistant specialized in video understanding and humor analysis. You can write high-quality descriptive caption based on the video content to attract viwers. """
            user_prompt = instruction['user_prompt']
    ...
    def post_process_response(self, task_type, raw_response):
        ...
        if task_type == "QA":
            ...
            return raw_response.replace("Caption: ", "").strip()
        elif task_type == "Process_QA":
            return raw_response
