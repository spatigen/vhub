...
class DREAMGPTMetric:
    ...
    def summarize_metric(self):
        ...
        has_extra_info = any(
            'extra_info' in (r.get('result') or {}) 
            for r in self.results
        )
        if has_extra_info:
            self._summarize_metric_by_bucket()
        else:
            print("\n[INFO] No 'extra_info' found. Skipping bucket analysis.")
        # self._summarize_metric_by_bucket()
    ...
    # 在 DREAMGPTMetric 类中添加
    def get_overall_scores(self):
        recalls = [r['result']['score_r'] for r in self.results if r['success']]
        precisions = [r['result']['score_p'] for r in self.results if r['success']]
        if not recalls or not precisions:
            return None, None, None
        avg_r = np.mean(recalls)
        avg_p = np.mean(precisions)
        f1 = count_f1(avg_r, avg_p)
        return avg_r, avg_p, f1

    @classmethod
    def from_csv_files(cls, 
                       ref_csv_path: str, 
                       cand_csv_path: str,
                       ref_column: str = "humor_explanation",
                       cand_column: str = "humor_explanation",
                       dataset_name: str = "default",
                       verbose: bool = False,
                       id_column: str = None):
        
        def _read_csv_column(csv_path, column, id_col=None):
            data = []
            ids = []
            with open(csv_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    if column not in row:
                        raise ValueError(f"Column '{column}' not found in {csv_path}. Available: {list(row.keys())}")
                    text = row[column].strip()
                    data.append(text)
                    if id_col:
                        ids.append(row.get(id_col, None))
            return data, ids

        # 读取 reference
        ref_texts, ref_ids = _read_csv_column(ref_csv_path, ref_column, id_column)
        # 读取 candidate
        cand_texts, cand_ids = _read_csv_column(cand_csv_path, cand_column, id_column)

        # 对齐数据
        if id_column is None:
            # 按行顺序对齐
            if len(ref_texts) != len(cand_texts):
                raise ValueError(f"Row count mismatch: ref={len(ref_texts)}, cand={len(cand_texts)}")
            aligned_pairs = list(zip(ref_texts, cand_texts))
        else:
            # 按 ID 对齐
            ref_dict = dict(zip(ref_ids, ref_texts))
            cand_dict = dict(zip(cand_ids, cand_texts))
            common_ids = [vid for vid in ref_ids if vid in cand_dict and vid is not None]
            if not common_ids:
                raise ValueError("No common IDs found between reference and candidate files.")
            aligned_pairs = [(ref_dict[vid], cand_dict[vid]) for vid in common_ids]

        # 创建实例
        metric = cls(dataset_name=dataset_name, verbose=verbose)
        
        # 添加数据（跳过空文本）
        for i, (ref, pred) in enumerate(aligned_pairs):
            if ref.strip() and pred.strip():
                metric.add({
                    ...
                    # 注意：如需分桶，需额外提供 extra_info（当前未实现）
                })
        return metric
...
def evaluate_from_csv(
    ...
    cand_csv_path: str,
    ref_column: str = "humor_explanation",
    cand_column: str = "explanation",
    id_column: str = None,
    dataset_name: str = "humor_ex",
    verbose: bool = False,
    save_dir: str = None
):
    
    
    print(f"[DREAM] Loading data from CSV files...")
    print(f"  Reference: {ref_csv_path} (column: {ref_column})")
    print(f"  Candidate: {cand_csv_path} (column: {cand_column})")
    
    try:
        # 使用类方法创建实例
        metric = DREAMGPTMetric.from_csv_files(
            ref_csv_path=ref_csv_path,
            cand_csv_path=cand_csv_path,
            ref_column=ref_column,
            cand_column=cand_column,
            id_column=id_column,
            dataset_name=dataset_name,
            verbose=verbose
        )
        
        if len(metric.dataset) == 0:
            print("[DREAM] No valid non-empty pairs found. Evaluation skipped.")
            return {"recall": 0.0, "precision": 0.0, "f1": 0.0, "success_count": 0, "failed_count": 0}
        
        print(f"[DREAM] Starting evaluation on {len(metric.dataset)} samples...")
        metric.process(metric.dataset)
        metric.summarize_metric()
        
        # # 保存详细结果（如果指定了目录）
        # if save_dir:
        #     metric.save_results(save_dir)
        #     metric.save_eval_infos(save_dir)
        
        # 获取指标
        recall, precision, f1 = metric.get_overall_scores()
        if recall is None:
        ...
        result = {
            ...
            "f1": float(f1),
            "success_count": len(metric.results),
            "failed_count": len(metric.invalid_results)
        }
        ...
        print(f"[DREAM ERROR] Evaluation failed: {e}")
        import traceback
        traceback.print_exc()
        return {"recall": 0.0, "precision": 0.0, "f1": 0.0, "success_count": 0, "failed_count": 0}
